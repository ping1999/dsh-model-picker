...
`
  ...
  // --- Per-session selection memory -----------------------------------------
  // The host keeps a selectModel result only in-process plus a global default
  // in settings; per-session restore after a host restart re-derives from the
  // session log's last request header, so an effort (or model) picked after
  // the session's last request silently reverts on restart. The panel
  // remembers the last settled selection per session and replays it once per
  // host generation whenever the host-restored snapshot diverges from it.
  const SELECTIONS_KEY = 'dsh-model-picker:selections:v1'
  const SELECTIONS_CAP = 200
  // Bumped on every connection/reset (host restart): a directory reconciled
  // under one generation is re-checked under the next. WeakMap keyed by the
  // per-session directory store, so disposed sessions drop out with it.
  let hostGeneration = 0
  const RECONCILED = new WeakMap()

  // Records come from localStorage: without string provider/model a record
  // can neither be compared nor replayed, so it reads as absent.
  ...
  function readSelections() {
    try {
      const raw = globalThis.localStorage === undefined ? null : globalThis.localStorage.getItem(SELECTIONS_KEY)
      if (typeof raw !== 'string') return {}
      const parsed = JSON.parse(raw)
      if (parsed === null || parsed.v !== 1 || parsed.sessions === null || typeof parsed.sessions !== 'object') return {}
      const out = {}
      for (const key of Object.keys(parsed.sessions)) {
        const record = cleanRecord(parsed.sessions[key])
  ...
  function rememberSelection(sessionId, current) {
    try {
      if (globalThis.localStorage === undefined) return
      const sessions = readSelections()
      sessions[sessionId] = {
        provider: current.provider,
        model: current.model,
        // The resolved effort is materialized by the host (a picked default
        // reads as the default's id); null marks an explicit provider-default
        // choice on a model without one.
        effort: typeof current.reasoningEffort === 'string' ? current.reasoningEffort : null,
        at: Date.now(),
      }
      const keys = Object.keys(sessions)
      if (keys.length > SELECTIONS_CAP) {
        keys.sort((a, b) => sessions[a].at - sessions[b].at)
        for (const key of keys.slice(0, keys.length - SELECTIONS_CAP)) delete sessions[key]
      }
      globalThis.localStorage.setItem(SELECTIONS_KEY, JSON.stringify({ v: 1, sessions }))
    } catch (err) { /* quota / privacy mode: the memory stays unwritten */ }
  }
  ...
  // selected — every consumer drops them: the catalog cleaner below, the
  // component's own guard for the never-cleaned per-session directory path,
  // and the selection-memory replay validation.
  ...
  // bare string, ...) reads as "no reasoning", and a non-string defaultEffort
  // is normalized away — left as-is it would ride straight into a select
  // payload and suppress the provider-default menu row.
  const cleanReasoning = (reasoning) => {
    if (reasoning === null || typeof reasoning !== 'object') return undefined
    return {
      ...reasoning,
      efforts: cleanEfforts(reasoning),
      defaultEffort: typeof reasoning.defaultEffort === 'string' ? reasoning.defaultEffort : undefined,
    }
  }
  ...
  function ModelPicker(props) {
    ...
    // The global catalog drives the list once it holds real data; an
    // empty/errored catalog never overrides a non-empty per-session
    // directory snapshot (its mount-time load returns the same groups), so a
    // premature empty answer can never blank the panel. When NEITHER source
    // has rows the catalog still owns the first-load signals: nothing loads
    // the directory on this path, so its status can sit at 'idle' forever
    // and the panel would paint a silent blank box — no loading row on a
    // cold first open, no retry row after a failed one, no warning rows when
    // every provider failed. Drive status/error/failures from the catalog in
    // that case.
    const catalogLive = catalog !== null && catalogState.groups.length > 0
    const catalogDrives = catalogLive || (catalog !== null && state.groups.length === 0)
    const groups = catalogDrives ? catalogState.groups : state.groups
    const failures = catalogDrives ? catalogState.failures : state.failures
    const listStatus = catalogDrives ? catalogState.status : state.status
    const listError = catalogDrives ? catalogState.error : state.error
    ...
    // Same guard as cleanReasoning for the never-cleaned directory path: a
    // non-string defaultEffort must neither become the effective effort nor
    ...
    // Per-session selection memory: persist the settled selection on the
    // selecting -> ready edge (the only transition a successful select
    // produces — covering both this panel and the built-in /model popup,
    // which share the directory), and replay the remembered selection once
    // per host generation when the host-restored snapshot diverges.
    useEffect(() => {
      if (!available || directory === null || sessionId === undefined) return
      const sid = String(sessionId)
      const reconcile = (snap) => {
        if (snap.status !== 'ready' || snap.current === null) return
        if (RECONCILED.get(directory) === hostGeneration) return
        const record = readSelections()[sid] ?? null
        const current = snap.current
        if (record === null
          || (record.provider === current.provider && record.model === current.model
            && (record.effort ?? undefined) === (current.reasoningEffort ?? undefined))) {
          RECONCILED.set(directory, hostGeneration)
          return
        }
        // Replay only when the catalog still serves the remembered route and
        // effort: otherwise the host's validation rejects the select and the
        // directory store latches an error status onto the panel.
        const group = snap.groups.find((g) => g.id === record.provider)
        const model = group === undefined ? undefined : group.models.find((m) => m.id === record.model)
        if (model === undefined) return
        if (record.effort !== null && !cleanEfforts(model.reasoning).some((level) => level.id === record.effort)) return
        RECONCILED.set(directory, hostGeneration)
        void select({
          provider: record.provider,
          model: record.model,
          ...(record.effort === null ? {} : { reasoningEffort: record.effort }),
        })
      }
      const initial = directory.getSnapshot()
      reconcile(initial)
      let prev = initial
      return directory.subscribe(() => {
        const next = directory.getSnapshot()
        if (prev.status === 'selecting' && next.status === 'ready' && next.current !== null) {
          rememberSelection(sid, next.current)
        } else if (next.status === 'ready' && next.current !== null && prev.status !== 'selecting') {
          reconcile(next)
        }
        prev = next
      })
    }, [available, directory, select, sessionId])
    ...
    const choose = (group, model) => {
      ...
      // (same normalization as cleanReasoning).
      const selection = {
        provider: group.id,
        model: model.id,
  ...
  function apply(ctx) {
    ...
    // A host restart resets every per-session directory (connection/reset);
    // bumping the generation re-arms the per-session reconcile in ModelPicker
    // so a stale host-restored selection is replayed once per generation.
    ctx.effect(() => {
      if (typeof ctx.on !== 'function') return undefined
      const ret = ctx.on('connection/reset', () => { hostGeneration += 1 })
      return typeof ret === 'function' ? ret : undefined
    }, 'model-picker-selection-memory')
      ...
      inject: (sessionId) => {
        try {
          ...
          if (models === undefined || typeof models.directoryFor !== 'function') {
            return { available: false, sessionId, directory: null, catalog: null, load: () => {}, select: () => Promise.resolve(false) }
          }
