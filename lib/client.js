...
`
  ...
  function readStorage() {
    try {
      ...
      if (parsed === null || parsed.v !== 1 || !Array.isArray(parsed.groups) || parsed.groups.length === 0) return null
      return {
  ...
  function ModelPicker(props) {
    ...
    useEffect(() => {
      ...
      if (real !== null && real.provider === optimistic.provider && real.model === optimistic.model
        && (optimistic.reasoningEffort === undefined || real.reasoningEffort === optimistic.reasoningEffort)) {
        setOptimistic(null)
      }
    }, [state, optimistic])
  ...
  function apply(ctx) {
    ...
    try {
      ...
      if (api !== undefined && api.llm !== undefined && typeof api.llm.models === 'function') {
        ...
        unref(setTimeout(() => {
          if (Date.now() - catalog.loadedAt() > CATALOG_TTL) void catalog.load()
        }, BOOT_WARMUP_DELAY))
      }
    } catch (err) {
      catalog = null
