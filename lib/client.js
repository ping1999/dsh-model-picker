    ...
    useEffect(() => {
      ...
      const reconcile = (snap) => {
        ...
        if (record.effort !== null) {
          const efforts = model.reasoning !== null && typeof model.reasoning === 'object' && Array.isArray(model.reasoning.efforts)
            ? model.reasoning.efforts : []
          if (!efforts.some((level) => level !== null && typeof level === 'object' && level.id === record.effort)) return
        }
        RECONCILED.set(directory, hostGeneration)
        void select({
