...
// other shows next. Opening the panel is instant: the store keeps the last
// loaded groups, so open only revalidates when empty / errored / older than
// RELOAD_TTL, and a background refresh never covers the cached list.
...
`
  ...
  // per-session snapshot store so the stamp survives seat remounts.
  const LOADED_AT = new WeakMap()
  // Open revalidates at most this often; llm/adapters-updated + settings
  // pushes and connection resets already refresh the directory out of band.
  ...
  function ModelPicker(props) {
    ...
    const show = () => {
      ...
      lastActionRef.current = 'load'
      setPane('models')
      // Instant open: only revalidate when there is nothing cached to show,
      // the last load failed, or the cache is older than RELOAD_TTL. A
      // background refresh keeps rendering the cached list (no loading row).
      const loadedAt = LOADED_AT.get(directory) ?? 0
      if (state.groups.length === 0 || state.status === 'error' || Date.now() - loadedAt > RELOAD_TTL) reload()
    }
